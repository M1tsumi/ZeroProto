...
impl<'a> MessageReader<'a> {
    ...
    fn field_entry(&self, field_index: u16) -> Result<(PrimitiveType, usize)> {
        ...
        let primitive_type = PrimitiveType::from_u8(type_id)
            .ok_or(Error::InvalidFieldType)?;

        Ok((primitive_type, offset))
    }

    /// Get a scalar field value
    pub fn get_scalar<T: ZpRead<'a>>(&self, field_index: u16) -> Result<T> {
        let (_, field_offset) = self.field_entry(field_index)?;
        T::read(self.buffer, field_offset)
    }

    /// Get a string field
    pub fn get_string(&self, field_index: u16) -> Result<&'a str> {
        let (field_type, field_offset) = self.field_entry(field_index)?;
    ...
    pub fn get_bytes(&self, field_index: u16) -> Result<&'a [u8]> {
        ...
        Ok(&self.buffer[bytes_offset..bytes_offset + len])
    }
    ...
    pub fn get_message(&self, field_index: u16) -> Result<MessageReader<'a>> {
        ...
        MessageReader::new(message_buffer)
    }
...
mod tests {
    ...
    fn test_invalid_message_too_short() {
        ...
        assert!(matches!(MessageReader::new(&buffer), Err(Error::InvalidMessage)));
    }
